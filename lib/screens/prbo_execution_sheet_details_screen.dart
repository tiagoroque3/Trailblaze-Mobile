...
class _ExecutionSheetDetailsScreenState
  ...
  void _addParcelOperation() {
    // TODO: Implement logic to add a new parcel-operation connection.
    // This would likely involve fetching available parcels and operations
    // from the original worksheet and showing a dialog.
    print('Add new parcel-operation connection');
  }
        ...
        child: FutureBuilder<List<ParcelOperationExecution>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return const Center(
                child: Text(
                    'No parcel operations found. Use the (+) button to add one.'),
              );
            } else {
              return ListView.builder(
                itemCount: snapshot.data!.length,
                itemBuilder: (context, index) {
                  final parcelOp = snapshot.data![index];
                  return Card(
